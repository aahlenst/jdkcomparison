...
describe("<LinuxSection/>", () => {
	...
	it("displays all features", () => {
		...
		linuxSection.expectFeatures(["x86, 64-bit", "x86, 64-bit, musl", "x86, 32-bit", "ARM, 64-bit", "ARM, 64-bit, musl", "ARM, 32-bit", "PPC, 64-bit", "RISC-V, 64-bit", "S390, 64-bit", "APK Packages", "Deb Packages", "RPM Packages", "Container Images"]);
		...
		linuxSection.expectFeaturePresence("linux-container-images", "Container Images", ["no", "no"]);
	});
	...
	it("displays ARM, 64-bit, musl if features are different and differences only is on", () => {
		...
		linuxSection.expectFeaturePresence("linux-aarch64-musl", "ARM, 64-bit, musl", ["yes", "no"]);
	});
	...
	it("displays Container Images if features are different and differences only is on", () => {
		...
		linuxSection.expectFeaturePresence("linux-container-images", "Container Images", ["partially", "no"]);
	});
});
...
const linuxSection = {
	...
	expectFeatures: (featureNames: string[]) => {
		cy.get("section[id='linux'] .feature .feature-name").should("have.length", featureNames.length);
