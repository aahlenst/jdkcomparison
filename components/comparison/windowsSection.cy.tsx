...
describe("<WindowsSection/>", () => {
	...
	it("displays all features", () => {
		cy.mount(<WindowsSection productData={data} showDifferencesOnly={false}/>);

		windowsSection.expectFeatures(["x86, 64-bit", "x86, 32-bit", "ARM, 64-bit", "Installers", "Container Images"]);
		windowsSection.expectFeaturePresence("windows-x64", "x86, 64-bit", ["no", "no"]);
		windowsSection.expectFeaturePresence("windows-aarch64", "ARM, 64-bit", ["no", "no"]);
		windowsSection.expectFeaturePresence("windows-installers", "Installers", ["no", "no"]);
		windowsSection.expectFeaturePresence("windows-container-images", "Container Images", ["no", "no"]);
	});
	...
	it("displays all features of a single product despite differences only on", () => {
		...
		windowsSection.expectFeatures(["x86, 64-bit", "x86, 32-bit", "ARM, 64-bit", "Installers", "Container Images"]);
		windowsSection.expectFeaturePresence("windows-x64", "x86, 64-bit", ["no"]);
		windowsSection.expectFeaturePresence("windows-aarch64", "ARM, 64-bit", ["no"]);
		windowsSection.expectFeaturePresence("windows-installers", "Installers", ["no"]);
		windowsSection.expectFeaturePresence("windows-container-images", "Container Images", ["no"]);
	});

	it("disappears if all features are identical and differences only is on", () => {
		cy.mount(<WindowsSection productData={data} showDifferencesOnly={true}/>);
	...
	it("displays x86, 64-bit if features are different and differences only is on", () => {
		...
		windowsSection.expectFeaturePresence("windows-x64", "x86, 64-bit", ["yes", "no"]);
	});
	...
	it("displays x86, 32-bit if features are different and differences only is on", () => {
		...
		windowsSection.expectFeaturePresence("windows-x32", "x86, 32-bit", ["yes", "no"]);
	});
	...
	it("displays ARM, 64-bit if features are different and differences only is on", () => {
		...
		windowsSection.expectFeaturePresence("windows-aarch64", "ARM, 64-bit", ["yes", "no"]);
	});
	...
	it("displays Installers if features are different and differences only is on", () => {
		...
		windowsSection.expectFeaturePresence("windows-installers", "Installers", ["yes", "no"]);
	});
	...
	it("displays Container Images if features are different and differences only is on", () => {
		...
		windowsSection.expectFeaturePresence("windows-container-images", "Container Images", ["partially", "no"]);
	});
});
...
const windowsSection = {
	...
	expectFeatures: (featureNames: string[]) => {
		cy.get("section[id='windows'] .feature .feature-name").should("have.length", featureNames.length);

		for (let i = 0; i < featureNames.length; i++) {
			const name = featureNames[i];
			cy.get("section[id='windows'] .feature .feature-name").eq(i).should("have.text", name);
		}
	},
