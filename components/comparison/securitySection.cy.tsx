...
describe("<SecuritySection/>", () => {
	...
	it("displays all features", () => {
		cy.mount(<SecuritySection productData={data} showDifferencesOnly={false} />);

		securitySection.expectFeatures(["SBOM"]);
		securitySection.expectFeaturePresence("security-sbom", "SBOM", ["no", "no"]);
	});
	...
	it("disappears if all features are identical and differences only is on", () => {
		cy.mount(<SecuritySection productData={data} showDifferencesOnly={true} />);
	...
	it("displays SBOM if features are different and differences only is on", () => {
		...
		securitySection.expectFeatures(["SBOM"]);
		securitySection.expectFeaturePresence("security-sbom", "SBOM", ["partially", "no"]);
	});
});
...
const securitySection = {
	...
	expectFeatures: (featureNames: string[]) => {
		...
		for (let i = 0; i < featureNames.length; i++) {
			...
			cy.get("section[id='security'] .feature .feature-name").eq(i).should("have.text", name);
		}
	},
