...
describe("<ComparisonSection/>", () => {
	const children = (
		<>
			<Feature id="feature-a" name="Feature A" values={[{id: "product-1", present: Present.YES}]}/>
			<Feature id="feature-b" name="Feature B" values={[{id: "product-1", present: Present.NO}]}/>
		</>
	);

	it("displays section with all features", () => {
		cy.mount(<ComparisonSection id="a-section" label="A section">{children}</ComparisonSection>);
...
const comparisonSectionComponent = {
	...
	expectFeaturesInSection: (sectionId: string, names: string[]) => {
		cy.get(`section[id='${sectionId}'] .feature .feature-name`).should("have.length", names.length);
