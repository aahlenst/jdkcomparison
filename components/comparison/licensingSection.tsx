...
export function LicensingSection({productData, showDifferencesOnly}: LicensingSectionProps) {
	const license = productData.map(product => ({...product.license, id: product.id}));
	const freeInDev = productData.map(product => ({...product.freeInDevelopment, id: product.id}));
	const freeInProd = productData.map(product => ({...product.freeInProduction, id: product.id}));
	...
	return (
		<ComparisonSection id="licensing" label="Licensing">
			...
			{showFeatures.freeInDev &&
				<Feature id="licensing-free-in-development" name="Free in Development" values={freeInDev}/>
			}
			{showFeatures.freeInProd &&
				<Feature id="licensing-free-in-production" name="Free in Production" values={freeInProd}/>
			}
		</ComparisonSection>
	);
}
