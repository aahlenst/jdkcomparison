...
export function FeaturePresence({present}: FeaturePresenceProps) {
	...
	switch (present) {
		...
		case Model.Present.PARTIALLY:
			presenceIcon = (
				<CircleHalfStrokeIcon className="present-partially h-4 w-4 inline" role="img" aria-label="partially">
		...
		case Model.Present.UNKNOWN:
			presenceIcon = (
				<QuestionIcon className="present-unknown h-4 w-4 inline" role="img" aria-label="unknown">
					<title>unknown</title>
				</QuestionIcon>
			);
			break;
