...
export function FeaturePresence({present}: FeaturePresenceProps) {
	...
	switch (present) {
		case Model.Present.YES:
			presenceIcon = <CircleIcon className="present-yes h-4 w-4 inline"/>;
			break;
		case Model.Present.PARTIALLY:
			presenceIcon = <CircleHalfStrokeIcon className="present-partially h-4 w-4 inline"/>;
			break;
		case Model.Present.NO:
			presenceIcon = <MinusIcon className="present-no h-4 w-4 inline"/>;
			break;
		case Model.Present.UNKNOWN:
			presenceIcon = <QuestionIcon className="present-unknown h-4 w-4 inline"/>;
			break;
		default:
