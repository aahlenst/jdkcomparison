...
The comparison distinguishes between two types of updates that vendors publish quarterly for their JDKs: Critical Patch Updates (CPU) and Patch Set Updates (PSU). [Oracle introduced those terms](https://www.java.com/releases/), but other vendors have adopted them in the meantime.
...
Please note that the exact definition of those terms might vary between vendors.
...
-   **tiered**: The availability of the type of update is somehow tiered. For example, it can be free of charge for a fixed timeframe since the initial release of the JDK. Further updates are only made available to paying customers.
